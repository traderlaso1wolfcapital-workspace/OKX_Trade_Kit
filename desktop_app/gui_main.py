...
PROJECT_DIR = _base
for _ in range(4):
    if os.path.isdir(os.path.join(PROJECT_DIR, "bots/sub1")):
        break
...
def auto_reset_state_on_update(data_dir, base_dir):
    try:
        ...
        if current_version != app_version:
            ...
            for b_name in ["bots/sub1", "bots/sub2"]:
            ...
            for i in range(1, 6):
                ...
                for c_name in cfg_names:
                    ...
                    if not os.path.exists(c_path):
                        try:
                            with open(c_path, "w", encoding="utf-8") as f:
                                json.dump(default_sub1_cfg if b_name == "bots/sub1" else default_sub2_cfg, f, indent=4)
                        except: pass
    ...
    def init_ui(self):
        ...
        def get_media_path(img_name):
            ...
            if os.path.exists(p1):
                return p1
            return os.path.join(PROJECT_DIR, "desktop_app", "media", img_name)
...
class LoginDialog(QtWidgets.QDialog):
    def __init__(self, parent=None):
        ...
        possible_banner_paths = [
            ...
            os.path.join(PROJECT_DIR, "desktop_app", "media", "banner.png"),
    ...
    if len(sys.argv) >= 4 and sys.argv[1] == '--run-bot':
        ...
        try:
            ...
            def load_and_run(module_name, file_name, env):
                ...
                try:
                    # Cấu trúc mới: bots.sub1.sys_bot_sub1
                    bot_id = module_name.split('_')[-1] # sub1
                    module = importlib.import_module(f"bots.{bot_id}.{module_name}")
                except ImportError:
                    try:
