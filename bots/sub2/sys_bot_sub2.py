...
# ==============================================================================
# Tự động thêm thư mục gốc dự án vào sys.path để hỗ trợ import bots.sub2 khi di chuyển file
_file_dir = os.path.dirname(os.path.abspath(__file__))
if os.path.basename(_file_dir) in ["bots/sub1", "bots/sub2"]:
    _parent_dir = os.path.dirname(_file_dir)
    if _parent_dir not in sys.path:
        sys.path.insert(0, _parent_dir)

from bots.sub2.bot_models import AssetTracker
from bots.sub2.bot_api import OKXRestCore
...
def main():
    ...
    if getattr(sys, 'frozen', False):
        ...
        for _ in range(4):
            if os.path.isdir(os.path.join(base_dir, "bots/sub2")):
                break
    ...
    possible_paths = [
        os.path.join(user_data_dir, "bots/sub2", env_basename),
        os.path.join(user_data_dir, "bots", "sub2", env_basename),
        os.path.join(user_data_dir, env_basename),
        os.path.join(base_dir, "bots/sub2", env_basename),
        os.path.join(base_dir, "bots", "sub2", env_basename),
    ...
    # 🔒 SINGLE INSTANCE LOCK — Ngăn chặn chạy 2 bot cùng tài khoản
    # =========================================================================
    sub2_dir = os.path.join(user_data_dir, "bots/sub2")
            ...
            try:
                ...
                if psutil.pid_exists(old_pid) and old_pid != os.getpid():
                    try:
                        ...
                        pass
                if os.path.exists(lock_file):
                    print(f"🧹 [LOCK] PID cũ ({old_pid}) đã ngắt. Tự động dọn dẹp lock cũ...")
                    os.remove(lock_file)
    ...
    # Luồng nhập Terminal
    import bots.sub2.bot_terminal as bot_terminal
    ...
    try:
        last_logic_mtime = os.path.getmtime(os.path.join(CURRENT_DIR, "bots/sub2", "bot_sub2.py"))
        last_ui_mtime = os.path.getmtime(os.path.join(CURRENT_DIR, "bots/sub2", "bot_ui.py"))
        last_cfg_mtime = os.path.getmtime(os.path.join(CURRENT_DIR, "bots/sub2", "bot_config.py"))
    ...
    import bots.sub2.bot_ui as bot_ui
    ...
    while True:
        try:
            ...
            # 2. HOT RELOADER
            try:
                curr_logic_mtime = os.path.getmtime(os.path.join(CURRENT_DIR, "bots/sub2", "bot_sub2.py"))
                curr_ui_mtime = os.path.getmtime(os.path.join(CURRENT_DIR, "bots/sub2", "bot_ui.py"))
                curr_cfg_mtime = os.path.getmtime(os.path.join(CURRENT_DIR, "bots/sub2", "bot_config.py"))
            except FileNotFoundError:
            ...
            if curr_logic_mtime > last_logic_mtime:
                try:
                    ...
                    for _mod_name in list(_sys.modules.keys()):
                        if 'bots/sub2' in _mod_name and _mod_name != 'bots.sub2.bot_api':
                            del _sys.modules[_mod_name]
                    import bots.sub2.bot_sub2 as bot_sub2  # re-import fresh
                    import bots.sub2.bot_config as bot_config  # re-import fresh
                    last_logic_mtime = curr_logic_mtime
                    print("\n🔄 Đã Hot-Reload bots.sub2.bot_sub2 thành công!")
                    # ⚡ TWO-WAY SYNC: Nếu bot_config.py mới hơn JSON → ghi đè JSON
                    json_config_path = env_paths.get("FILE_GLOBAL_CONFIG", "")
                    cfg_py_path = os.path.join(CURRENT_DIR, "bots/sub2", "bot_config.py")
            ...
            if curr_ui_mtime > last_ui_mtime:
                try:
                    ...
                    print("\n🔄 Đã Hot-Reload bots.sub2.bot_ui thành công!")
            ...
            if curr_cfg_mtime > last_cfg_mtime:
                try:
                    ...
                    print("\n🔄 Đã Hot-Reload bots.sub2.bot_config thành công!")
                    # ⚡ TWO-WAY SYNC: Nếu bot_config.py mới hơn JSON → ghi đè JSON
                    json_config_path = env_paths.get("FILE_GLOBAL_CONFIG", "")
                    cfg_py_path = os.path.join(CURRENT_DIR, "bots/sub2", "bot_config.py")
                    if os.path.exists(cfg_py_path) and os.path.exists(json_config_path):
