...
class BotInstanceWidget(QtWidgets.QWidget):
    ...
    def set_welcome_name(self, name):
        lbl = QtWidgets.QLabel(f" Chúc \"{name}\" giao dịch thuận lợi ")
        lbl.setStyleSheet("color: #ffaa00; font-size: 13px; font-weight: bold; font-style: italic; margin-right: 20px; margin-top: 5px;")
        self.tab_live_view.setCornerWidget(lbl, QtCore.Qt.Corner.TopRightCorner)
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def init_ui(self):
        ...
        btn_discord.setStyleSheet("background: transparent; border: none;")
        ...
        header_layout.addWidget(btn_discord)
...
class HWIDAuthDialog(QtWidgets.QDialog):
    def __init__(self, hwid, parent=None):
        ...
        lbl_msg1 = QtWidgets.QLabel("Tài khoản hợp lệ, nhưng CHƯA được cấp quyền sử dụng trên máy tính này.")
        lbl_msg1.setWordWrap(True)
        layout.addWidget(lbl_msg1)
        
        hwid_layout = QtWidgets.QHBoxLayout()
        lbl_msg2 = QtWidgets.QLabel("Mã Máy (HWID) của bạn: ")
        ...
        lbl_msg3 = QtWidgets.QLabel("Vui lòng click vào Mã Máy trên để Copy và gửi cho Admin để được cấp quyền!")
            ...
            if user_info is not None:
                ...
                if not registered_hwid:
                    ...
                    QtWidgets.QMessageBox.critical(
                        self,
                        "Sai thiết bị truy cập",
                        f"Tài khoản UID này đã được cấp quyền sử dụng cho một máy tính khác!\n\n"
                        f"Không thể sử dụng chung 1 tài khoản cho nhiều người/nhiều máy.\n"
                        f"Nếu bạn đổi máy tính, vui lòng liên hệ Admin để reset Mã Thiết Bị."
                    )
                    self.btn_login.setText("Đăng Nhập")
                    self.btn_login.setEnabled(True)
                    return
